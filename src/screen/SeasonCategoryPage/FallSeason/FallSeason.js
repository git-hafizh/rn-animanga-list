...
const FallSeason = () => {
  ...
  useEffect(() => {
    ...
    const getAnimeFall = () => {
      axios.get(GET_SEASON_ANIME + "/fall", { cancelToken: source.token }).then((result) => {
        setFall(result.data.anime)
        setLoading(false)
      })
        .catch((err) => {
          console.log(err)
  ...
  return (
    <SafeAreaView style={Styles.containerSection}>
      {loading ? <LoadingIndicator/> : fall.length > 0 ?
        <FlatList
          data={fall}
          renderItem={({ item }) =>
