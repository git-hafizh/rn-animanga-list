...
const WinterSeason = () => {
  ...
  useEffect(() => {
    ...
    const getAnimeWinter = () => {
      axios.get(GET_SEASON_ANIME + "/winter", { cancelToken: source.token }).then((result) => {
        setWinter(result.data.anime)
        setLoading(false)
      })
        .catch((err) => {
          console.log(err)
  ...
  return (
    <SafeAreaView style={Styles.containerSection}>
      {loading ? <LoadingIndicator/> : winter.length > 0 ?
        <FlatList
          data={winter}
          renderItem={({ item }) =>
