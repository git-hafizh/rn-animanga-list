...
const SpringSeason = () => {
  ...
  useEffect(() => {
    ...
    const getAnimeSpring = () => {
      axios.get(GET_SEASON_ANIME + "/spring", { cancelToken: source.token }).then((result) => {
        setSpring(result.data.anime)
        setLoading(false)
      })
        .catch((err) => {
          console.log(err)
  ...
  return (
    <SafeAreaView style={Styles.containerSection}>
      {loading ? <LoadingIndicator/> : spring.length > 0 ?
        <FlatList
          data={spring}
          renderItem={({ item }) =>
