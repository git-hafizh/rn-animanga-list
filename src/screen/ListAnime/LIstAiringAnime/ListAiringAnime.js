...
import { GET_AIRED_ANIME, GET_EPISODES_ANIME, GET_SCORE_ANIME, GET_TOP_AIRING_ANIME, GET_TYPE_ANIME } from '../../../constants/constants';
import { Rating } from 'react-native-ratings';
import { LoadingIndicator } from '../../../helper';

const ListAiringAnime = () => {
  const [listAiring, setListAiring] = useState([]);
  ...
  return (
    <SafeAreaView style={Styles.containerSection}>
      {loading ? <LoadingIndicator /> : listAiring.length > 0 ?
        <FlatList
          data={listAiring}
          renderItem={({ item }) =>
...
export default ListAiringAnime
